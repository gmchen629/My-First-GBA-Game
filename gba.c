...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  // TODO: IMPLEMENT
  volatile unsigned short color_temp = 0;
  color_temp = color;
  for(int cur_row = 0; cur_row < height; cur_row++) {
    DMA[3].src = &color_temp;
    DMA[3].dst = &videoBuffer[OFFSET(cur_row + row, col, WIDTH)];
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  // TODO: IMPLEMENT
  for (int cur_row = 0; cur_row < height; cur_row++) {
    DMA[3].src = &image[OFFSET(cur_row, 0, width)];
    DMA[3].dst = &videoBuffer[OFFSET(cur_row + row, col, WIDTH)];
    DMA[3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT;
  }
}
...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  // TODO: IMPLEMENT
  for (int cur_row = 0; cur_row < height; cur_row++) {
    DMA[3].src = &image[OFFSET(cur_row + row, col, WIDTH)];
    DMA[3].dst = &videoBuffer[OFFSET(cur_row + row, col, WIDTH)];
    DMA[3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT;
  }
}
